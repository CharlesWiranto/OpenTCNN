"""This is the file containing kernels used for MinMaxPlusSumConv2d operation."""
...
def min_max_plus_sum_conv2d_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def main(
            ...
            output1_buffer : T.Buffer((batch_size, out_channels, out_height, out_width), dtype),
            output2_buffer : T.Buffer((batch_size, out_channels, out_height, out_width), dtype),
        ):
            ...
            with T.Kernel(bo_total, SPATIAL_TILES, threads=(BLOCK_P, reduce_threads)) as (bo, tile_id):
                    ...
                    if valid_out:
                        ...
                        for ic in T.serial(TILE_IC):
                            ...
                            if ic_global < in_channels:
                                ...
                                local_accum[1] += local_min[0]
                    T.sync_threads()

                sh_sum[tp, tic, 0] = local_accum[0]
                sh_sum[tp, tic, 1] = local_accum[1]
                T.sync_threads()
                ...
                if tic == 0 and valid_out:
                    ...
                    for r in T.serial(reduce_threads):
                        total[0] += sh_sum[tp, r, 0]
                        total[1] += sh_sum[tp, r, 1]
                    output1_buffer[b, oc, oh, ow] = total[0]
                    output2_buffer[b, oc, oh, ow] = total[1]
...
def min_max_plus_sum_conv2d_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return min_max_plus_sum_conv2d_kernel(
        batch_size,
        in_channels,
        out_channels,
        in_height,
...
# @tilelang.jit
def min_max_plus_sum_conv2d_kernel_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def compute_dweight(
            img_buffer: T.Buffer((batch_size, in_channels, in_height, in_width), dtype),
            weight_buffer: T.Buffer((out_channels, in_channels, kernel_height, kernel_width), dtype),
            dout1_buffer: T.Buffer((batch_size, out_channels, out_height, out_width), dtype),
            dout2_buffer: T.Buffer((batch_size, out_channels, out_height, out_width), dtype),
            ...
            with T.Kernel(
                ...
                sh_dout = T.alloc_shared((reduce_threads, BLOCK_OC, 2), dtype)
                ...
                for bhw_it in T.serial(T.ceildiv(BHW, reduce_threads)):
                    ...
                    if bhw0 < BHW:
                        ...
                        if tic == 0 and toc < BLOCK_OC:
                            ...
                            if ocg < out_channels:
                                sh_dout[tr, toc, 0] = dout1_buffer[b, ocg, oh, ow]
                                sh_dout[tr, toc, 1] = dout2_buffer[b, ocg, oh, ow]
                            else:
                                sh_dout[tr, toc, 0] = T.cast(0, dtype)
                                sh_dout[tr, toc, 1] = T.cast(0, dtype)
                    T.sync_threads()
                                ...
                                for kw in T.serial(kernel_width):
                                    if (kmax[0] > 0) and (vals[kh, kw] == vmax[0]):
                                        local_dw[kh, kw] = local_dw[kh, kw] + (
                                            sh_dout[tr, toc, 0] / T.cast(kmax[0], dtype)
                                        )
                                    if (kmin[0] > 0) and (-vals[kh, kw] == vnegmax[0]):
                                        local_dw[kh, kw] = local_dw[kh, kw] + (
                                            sh_dout[tr, toc, 1] / T.cast(kmin[0], dtype)
                                        )
                        else:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    ...
                                    if (-vals[kh, kw] == vnegmax[0]) and (min_h[0] == -1):
                                        ...
                                        min_w[0] = kw
                            if max_h[0] >= 0:
                                local_dw[max_h[0], max_w[0]] = local_dw[max_h[0], max_w[0]] + sh_dout[tr, toc, 0]
                            if min_h[0] >= 0:
                                local_dw[min_h[0], min_w[0]] = local_dw[min_h[0], min_w[0]] + sh_dout[tr, toc, 1]
                    T.sync_threads()
...
# @tilelang.jit
def min_max_plus_sum_conv2d_kernel_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def compute_dinput(
            ...
            with T.Kernel(bo_total, SPATIAL_TILES, threads=(BLOCK_P, reduce_threads)) as (bo, tile_id):
                ...
                linear_tid = tic * BLOCK_P + tp
                total_threads = BLOCK_P * reduce_threads
                local_dout = T.alloc_local((2,), dtype)
                if valid:
                    local_dout[0] = dout1_buffer[b, oc, oh, ow]
                    local_dout[1] = dout2_buffer[b, oc, oh, ow]
                else:
                    local_dout[0] = T.cast(0, dtype)
                    local_dout[1] = T.cast(0, dtype)
                                ...
                                if split_ties:
                                            ...
                                            if (ih >= 0) and (ih < in_height) and (iw >= 0) and (iw < in_width):
                                                if (kmax[0] > 0) and (vals[kh, kw] == vmax[0]):
                                                    T.atomic_add(
                                                        ...
                                                        local_dout[0] / T.cast(kmax[0], dtype),
                                                    )
                                                if (kmin[0] > 0) and (-vals[kh, kw] == vnegmax[0]):
                                                    T.atomic_add(
                                                        dimg_buffer[b, icg, ih, iw],
                                                        local_dout[1] / T.cast(kmin[0], dtype),
                                                    )
                                else:
                                    max_h = T.alloc_local((1,), int_dtype)
                                    max_w = T.alloc_local((1,), int_dtype)
                                    ...
                                    if max_h[0] >= 0:
                                        ...
                                        if (ih1 >= 0) and (ih1 < in_height) and (iw1 >= 0) and (iw1 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih1, iw1], local_dout[0])
                                    if min_h[0] >= 0:
                                        ...
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], local_dout[1])
                    T.sync_threads()
...
# @tilelang.jit
def min_max_plus_sum_conv2d_kernel_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return min_max_plus_sum_conv2d_kernel_backward_dinput(
        batch_size,
        in_channels,
        out_channels,
        in_height,
