"""Warp for MinPlusSumConv1d.

Forward uses `min_plus_sum_conv1d_kernel` (NCL input, OIK weight, NCL output).
...
from .min_plus_sum_conv1d import (
    min_plus_sum_conv1d_kernel,
    min_plus_sum_conv1d_kernel_backward_dweight,
    min_plus_sum_conv1d_kernel_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return min_plus_sum_conv1d_kernel(*key)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return min_plus_sum_conv1d_kernel_backward_dweight(*key, maxmin_gradient=maxmin_gradient)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return min_plus_sum_conv1d_kernel_backward_dinput(*key, maxmin_gradient=maxmin_gradient)


def clear_min_plus_sum_conv1d_kernel_cache():
    """Clear Python-side compilation caches for MinPlusSumConv1d kernels."""
...
class _MinPlusSumConv1d(autograd.Function):
    """Autograd function for the MinPlusSumConv1d operator."""
...
class MinPlusSumConv1d(_Conv):
    """Min-Plus-Sum 1D Convolutional Layer."""
    ...
    def _conv_forward(
        ...
        output = _MinPlusSumConv1d.apply(
            input,
            weight,
            self,
            input.shape[0],
