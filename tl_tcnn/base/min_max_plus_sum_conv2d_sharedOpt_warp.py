"""Warp for MinMaxPlusSumConv2d.

Forward uses `min_max_plus_sum_conv2d_kernel` (NCHW input, OIHW weight, NCHW output).
...
from .min_max_plus_sum_conv2d_sharedOpt import (
    min_max_plus_sum_conv2d_kernel,
    min_max_plus_sum_conv2d_kernel_backward_dweight,
    min_max_plus_sum_conv2d_kernel_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return min_max_plus_sum_conv2d_kernel(*key)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return min_max_plus_sum_conv2d_kernel_backward_dweight(*key, maxmin_gradient=maxmin_gradient)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return min_max_plus_sum_conv2d_kernel_backward_dinput(*key, maxmin_gradient=maxmin_gradient)


def clear_min_max_plus_sum_conv2d_kernel_cache():
    """Clear Python-side compilation caches for MinMaxPlusSumConv2d kernels."""
...
class _MinMaxPlusSumConv2d(autograd.Function):
    """Autograd function for the MinMaxPlusSumConv2d operator."""
    ...
    def forward(
        ...
        conv_module._run_forward_kernel(input_nchw, weight_oihw, output1, output2, out_height, out_width)
        ...
        return output1.contiguous(), output2.contiguous()
    ...
    def backward(ctx, grad_output1: Tensor, grad_output2: Tensor):
        ...
        grad_output1_nchw = grad_output1.contiguous()
        grad_output2_nchw = grad_output2.contiguous()
...
class MinMaxPlusSumConv2d(_Conv):
    """Max-Plus-Sum 2D Convolutional Layer with 2 Parameters."""
    ...
    def __init__(
        ...
        clear_min_max_plus_sum_conv2d_kernel_cache()
    ...
    def _run_forward_kernel(self, input_nchw, weight_oihw, output1, output2, out_height, out_width):
        ...
        if key not in self.forward_kernels:
            with set_autotune_inputs(input_nchw, weight_oihw, output1, output2):
                self.forward_kernels[key] = _compile_forward_kernel_cached(*key)
        self.forward_kernels[key](input_nchw, weight_oihw, output1, output2)
    ...
    def _run_backward_kernel(
        ...
        if key not in self.backward_kernels_dweight:
            with set_autotune_inputs(input_nchw, weight_oihw, grad_output1_nchw, grad_output2_nchw, grad_weight_oihw):
        ...
        if key not in self.backward_kernels_dinput:
            with set_autotune_inputs(input_nchw, weight_oihw, grad_output1_nchw, grad_output2_nchw, grad_input_nchw):
    ...
    def _conv_forward(
        ...
        output1, output2 = _MinMaxPlusSumConv2d.apply(
            input,
            weight,
            self,
            input.shape[0],
        ...
        if bias is not None:
            output1 += bias.view(1, -1, 1, 1)
            output2 += bias.view(1, -1, 1, 1)
        return output1, output2

    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the MinMaxPlusSumConv2d layer."""
