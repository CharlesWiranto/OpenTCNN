...
def max_plus_max_conv2d_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
def max_plus_max_conv2d_kernel_save_idx(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def max_plus_max_conv2d_kernel_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                    ...
                    for bhw_block in T.serial(T.ceildiv(BHW, BLOCK_BHW)):
                        ...
                        for bhw_offset in T.serial(TILE_BHW):
                            ...
                            if bhw < BHW:
                                ...
                                # 3) find max over channel-wise maxima and its multiplicity
...
# @tilelang.jit
def max_plus_max_conv2d_kernel_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel_dinput_multi(
        ...
        def compute_dinput_multi(
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P),
                ...
                # final channel-wise max (over per-channel maxplus)
                final_max = T.alloc_local((1,), dtype)
                max_ch_k = T.alloc_local((1,), int_dtype)
                ...
                if valid_b and valid_patch:
                    ...
                    for ic in T.serial(in_channels):
                        if max_val[ic] > final_max[0]:
                            final_max[0] = max_val[ic]
                            max_ch_k[0] = 1
                        elif max_val[ic] == final_max[0]:
                            max_ch_k[0] += 1
                                    ...
                                    if in_h < in_height and in_w < in_width:
                                        if (
                                            ...
                                            T.atomic_add(
                                                ...
                                                local_dout[0] / (T.cast(max_k[ic], dtype) * T.cast(max_ch_k[0], dtype)),
                                            )
...
def max_plus_max_conv2d_kernel_backward_all(
    batch_size,
    in_channels,
    out_channels,
    in_height,
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P),
                ...
                if valid_b and valid_patch:
                    ...
                    for ic in T.serial(in_channels):
                        ...
                        if max_val[0] > final_max[0]:
                            final_max[0] = max_val[0]
                            sel_ic[0] = ic
                            sel_h1[0] = max_h0[0]
                            sel_w1[0] = max_w0[0]
