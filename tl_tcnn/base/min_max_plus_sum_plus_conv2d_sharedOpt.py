...
def min_max_plus_sum_plus_conv2d_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def main(
            ...
            with T.Kernel(bo_total, SPATIAL_TILES, threads=(BLOCK_P, reduce_threads)) as (bo, tile_id):
                ...
                sh_sum = T.alloc_shared((BLOCK_P, reduce_threads, 1), dtype)
                    ...
                    if valid_out:
                        ...
                        for ic in T.serial(TILE_IC):
                            ...
                            if ic_global < in_channels:
                                ...
                                local_accum[0] += local_max[0] + local_min[0]
                    T.sync_threads()
...
def min_max_plus_sum_plus_conv2d_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return min_max_plus_sum_plus_conv2d_kernel(
        batch_size,
        in_channels,
        out_channels,
        in_height,
...
# @tilelang.jit
def min_max_plus_sum_plus_conv2d_kernel_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def compute_dweight(
            ...
            with T.Kernel(
                ...
                sh_dout = T.alloc_shared((reduce_threads, BLOCK_OC, 1), dtype)
                ...
                for bhw_it in T.serial(T.ceildiv(BHW, reduce_threads)):
                    ...
                    if bhw0 < BHW:
                        ...
                        if tic == 0 and toc < BLOCK_OC:
                            ...
                            if ocg < out_channels:
                                sh_dout[tr, toc, 0] = dout_buffer[b, ocg, oh, ow]
                            else:
                    ...
                    if valid and bhw0 < BHW:
                        ...
                        if split_ties:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    ...
                                    if (kmin[0] > 0) and (-vals[kh, kw] == vnegmax[0]):
                                        local_dw[kh, kw] = local_dw[kh, kw] + (
                                            sh_dout[tr, toc, 0] / T.cast(kmin[0], dtype)
                                        )
                        else:
                            ...
                            if min_h[0] >= 0:
                                local_dw[min_h[0], min_w[0]] = local_dw[min_h[0], min_w[0]] + sh_dout[tr, toc, 0]
                    T.sync_threads()
...
# @tilelang.jit
def min_max_plus_sum_plus_conv2d_kernel_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                                ...
                                if split_ties:
                                        ...
                                        for kw in T.serial(kernel_width):
                                            ...
                                            if (ih >= 0) and (ih < in_height) and (iw >= 0) and (iw < in_width):
                                                ...
                                                if (kmin[0] > 0) and (-vals[kh, kw] == vnegmax[0]):
                                                    T.atomic_add(
                                                        dimg_buffer[b, icg, ih, iw],
                                                        local_dout[0] / T.cast(kmin[0], dtype),
                                                    )
                                else:
                                    max_h = T.alloc_local((1,), int_dtype)
                                    max_w = T.alloc_local((1,), int_dtype)
                                    ...
                                    if min_h[0] >= 0:
                                        ...
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], local_dout[0])
                    T.sync_threads()
...
# @tilelang.jit
def min_max_plus_sum_plus_conv2d_kernel_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return min_max_plus_sum_plus_conv2d_kernel_backward_dinput(
        batch_size,
        in_channels,
        out_channels,
        in_height,
...
# @tilelang.jit
def min_max_plus_sum_plus_conv2d_kernel_backward_dinput_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                            ...
                            if icg < in_channels:
                                ...
                                if split_ties:
                                    ...
                                    if max_h[0] >= 0:
                                        ...
                                        if (ih1 >= 0) and (ih1 < in_height) and (iw1 >= 0) and (iw1 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih1, iw1], local_dout[0])
                                            T.atomic_add(dweight_buffer[oc, icg, max_h[0], max_w[0]], local_dout[0])
                                    if min_h[0] >= 0:
                                        ih2 = oh * stride_h + min_h[0] * dilation_h
                                        iw2 = ow * stride_w + min_w[0] * dilation_w
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], local_dout[0])
                                            T.atomic_add(dweight_buffer[oc, icg, min_h[0], min_w[0]], local_dout[0])
                    T.sync_threads()
