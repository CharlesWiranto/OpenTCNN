                    ...
                    if valid_out:
                        ...
                        for ic in T.serial(TILE_IC):
                            ...
                            if ic_global < in_channels:
                                ...
                                local_accum[0] += (local_max[0] if local_max[0] > T.cast(-T.infinity(dtype), dtype) else T.cast(0, dtype))
                    T.sync_threads()
...
def max_plus_sum_conv2d_kernel_tiled(
    ...
    return max_plus_sum_conv2d_kernel(
        batch_size,
        in_channels,
        out_channels,
        in_height,
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (kmax[0] > 0) and (vals[kh, kw] == vmax[0]):
                                        local_dw[kh, kw] += (
                                            sh_dout[tr, toc] / T.cast(kmax[0], dtype)
                                        )
                        else:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (vals[kh, kw] == vmax[0]) and (max_h[0] == -1):
                                        ...
                                        max_w[0] = kw
                            if max_h[0] >= 0:
                                local_dw[max_h[0], max_w[0]] += sh_dout[tr, toc]
                    T.sync_threads()
...
# @tilelang.jit
def max_plus_sum_conv2d_kernel_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return max_plus_sum_conv2d_kernel_backward_dinput(
        batch_size,
        in_channels,
        out_channels,
        in_height,
