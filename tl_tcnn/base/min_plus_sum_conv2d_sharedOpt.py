...
def min_plus_sum_conv2d_kernel_tiled(
    ...
    return min_plus_sum_conv2d_kernel(
        batch_size,
        in_channels,
        out_channels,
        in_height,
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (kmin[0] > 0) and (-vals[kh, kw] == vnegmax[0]):
                                        local_dw[kh, kw] += (sh_dout[tr, toc] / T.cast(kmin[0], dtype))
                        else:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (-vals[kh, kw] == vnegmax[0]) and (min_h[0] == -1):
                                        ...
                                        min_w[0] = kw
                            if min_h[0] >= 0:
                                local_dw[min_h[0], min_w[0]] += sh_dout[tr, toc]
                    T.sync_threads()
...
# @tilelang.jit
def min_plus_sum_conv2d_kernel_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return min_plus_sum_conv2d_kernel_backward_dinput(
        batch_size,
        in_channels,
        out_channels,
        in_height,
