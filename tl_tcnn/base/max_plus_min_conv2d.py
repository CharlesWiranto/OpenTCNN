...
def max_plus_min_conv2d_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                    ...
                    for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                        ...
                        for ic in T.serial(TILE_IC):
                            ...
                            if ic_global < in_channels:
                                ...
                                if local_max[0] < local_accum[0]: # min operation
                                    local_accum[0] = local_max[0]
...
def max_plus_min_conv2d_kernel_save_idx(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def max_plus_min_conv2d_kernel_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def max_plus_min_conv2d_kernel_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
def max_plus_min_conv2d_kernel_backward_all(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel_single(
        ...
        def compute_dinput_dweight_single(
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P),
                ...
                # local memory
                max_val = T.alloc_local((1,), dtype)
                max_h0 = T.alloc_local((1,), int_dtype)
                max_w0 = T.alloc_local((1,), int_dtype)
                        ...
                        for kh in T.serial(kernel_height):
                            for kw in T.serial(kernel_width):
                                ...
                                if in_h < in_height and in_w < in_width:
                                    ...
                                    if plus_val[0] > max_val[0]:
                                        max_val[0] = plus_val[0]
                                        max_h0[0] = kh
                                        max_w0[0] = kw
                        
                        if -max_val[0] > neg_max_val[0]:
                            neg_max_val[0] = -max_val[0]
                            min_ic[0] = ic
                            min_h1[0] = max_h0[0]
                            min_w1[0] = max_w0[0]
