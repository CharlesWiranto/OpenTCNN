"""Warp for MaxPlusSumConv3d.

Forward uses `max_plus_sum_conv3d_kernel` (NCDHW input, OIDHW weight, NCDHW output).
...
from .max_plus_sum_conv3d import (
    max_plus_sum_conv3d_kernel,
    max_plus_sum_conv3d_kernel_backward_dweight,
    max_plus_sum_conv3d_kernel_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return max_plus_sum_conv3d_kernel(*key)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return max_plus_sum_conv3d_kernel_backward_dweight(*key, maxmin_gradient=maxmin_gradient)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return max_plus_sum_conv3d_kernel_backward_dinput(*key, maxmin_gradient=maxmin_gradient)


def clear_max_plus_sum_conv3d_kernel_cache():
    """Clear Python-side compilation caches for MaxPlusSumConv3d kernels."""
...
class _MaxPlusSumConv3d(autograd.Function):
    """Autograd function for the MaxPlusSumConv3d operator."""
...
class MaxPlusSumConv3d(_Conv):
    """Max-Plus-Sum 3D Convolutional Layer."""
    ...
    def _conv_forward(self, input: Tensor, weight: Tensor, bias: Optional[Tensor]) -> Tensor:
        ...
        output = _MaxPlusSumConv3d.apply(
            input,
            weight,
            self,
            input.shape[0],
