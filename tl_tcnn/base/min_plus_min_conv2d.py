...
def min_plus_min_conv2d_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        ...
                                        if in_h < in_height and in_w < in_width:
                                            ...
                                            if val < local_min[0]:
                                                local_min[0] = val
                                # local_accum[0] += local_min[0] 
                                if local_min[0] < local_accum[0]: # min operation
                                    local_accum[0] = local_min[0]
...
def min_plus_min_conv2d_kernel_save_idx(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def main(
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P), 
                ...
                local_min_idx = T.alloc_local((3,), int_dtype) # (ic, kh, kw)
                local_accum_idx = T.alloc_local((3,), int_dtype) # (ic, kh, kw)
                T.clear(local_min_idx)
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        ...
                                        if in_h < in_height and in_w < in_width:
                                            ...
                                            if val < local_min[0]:
                                                local_min[0] = val
                                                local_min_idx[0] = ic_global
                                                local_min_idx[1] = kh
                                                local_min_idx[2] = kw
                                # local_accum[0] += local_min[0] 
                                if local_min[0] < local_accum[0]: # min operation
                                    local_accum[0] = local_min[0]
                                    local_accum_idx[0] = local_min_idx[0]
                                    local_accum_idx[1] = local_min_idx[1]
                                    local_accum_idx[2] = local_min_idx[2]
...
# @tilelang.jit
def min_plus_min_conv2d_kernel_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                    ...
                    for bhw_block in T.serial(T.ceildiv(BHW, BLOCK_BHW)):
                        ...
                        for bhw_offset in T.serial(TILE_BHW):
                            ...
                            if bhw < BHW:
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        ...
                                        if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width):
                                            ...
                                            if local_val[kh, kw] < min_val[0]:
                                                min_val[0] = local_val[kh, kw]
                                ...
                                # 3) find min over channel-wise maxima and its multiplicity
                                final_min = T.alloc_local((1,), dtype)
                                min_k = T.alloc_local((1,), int_dtype)
                                final_min[0] = T.infinity(dtype)
                                min_k[0] = 0
                                    ...
                                    for kh in T.serial(kernel_height):
                                        for kw in T.serial(kernel_width):
                                            ...
                                            if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width):
                                                ...
                                                if v2 < ch_max[0]:
                                                    ch_max[0] = v2
                                    if ch_max[0] < final_min[0]:
                                        final_min[0] = ch_max[0]
                                        min_k[0] = 1
                                    elif ch_max[0] == final_min[0]:
                                        min_k[0] += 1
                                
                                # 4) route gradient only if this channel contributes to min over channels
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        in_h = oh_stride + kh * dilation_h
                                        in_w = ow_stride + kw * dilation_w
                                        ...
                                        if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width):
                                            if (
                                                (max_k[0] > 0)
                                                & (min_k[0] > 0)
                                                & (min_val[0] == final_min[0])
                                                & (local_val[kh, kw] == min_val[0])
                                            ):
                                                local_dweight_acc[kh, kw] += (
                                                    local_dout[bhw_offset]
                                                    / (T.cast(max_k[0], dtype) * T.cast(min_k[0], dtype))
                                                )
...
# @tilelang.jit
def min_plus_min_conv2d_kernel_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel_dinput_multi(
        ...
        def compute_dinput_multi(
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P),
                ...
                # local memory
                min_val = T.alloc_local((in_channels,), dtype)
                max_k = T.alloc_local((in_channels,), int_dtype)
                
                T.fill(min_val, T.infinity(dtype))
                T.fill(max_k, 0)
                ...
                if valid_b and valid_patch:
                        ...
                        for kh in T.serial(kernel_height):
                            for kw in T.serial(kernel_width):
                                ...
                                if in_h < in_height and in_w < in_width:
                                    ...
                                    if plus_val[ic, kh, kw] < min_val[ic]:
                                        min_val[ic] = plus_val[ic, kh, kw]
                    ...
                    for ic in T.serial(in_channels):
                        if min_val[ic] < final_min[0]:
                            final_min[0] = min_val[ic]
                            min_k[0] = 1
                        elif min_val[ic] == final_min[0]:
                            min_k[0] += 1
                        ...
                        if min_val[ic] == final_min[0]:
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    ...
                                    if in_h < in_height and in_w < in_width:
                                        if (
                                            (plus_val[ic, kh, kw] == min_val[ic])
                                            and (max_k[ic] > 0)
                                            and (min_k[0] > 0)
                                        ):
                                            T.atomic_add(
                                                dimg_buffer[batch_idx, ic, in_h, in_w],
                                                local_dout[0] / (T.cast(max_k[ic], dtype) * T.cast(min_k[0], dtype)),
                                            )
...
def min_plus_min_conv2d_kernel_backward_all(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel_single(
        ...
        def compute_dinput_dweight_single(
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P),
                ...
                final_neg_max = T.alloc_local((1,), dtype)
                min_ic = T.alloc_local((1,), int_dtype)
                min_h1 = T.alloc_local((1,), int_dtype)
                min_w1 = T.alloc_local((1,), int_dtype)

                final_neg_max[0] = -T.infinity(dtype)
                ...
                if valid_b and valid_patch:
                    ...
                    for ic in T.serial(in_channels):
                        ...
                        # final min over channels via neg-max
                        if neg_max_val[0] > final_neg_max[0]:
                            final_neg_max[0] = neg_max_val[0]
                            min_ic[0] = ic
                            min_h1[0] = min_h0[0]
                            min_w1[0] = min_w0[0]
                            
                    in_h = oh_stride + min_h1[0] * dilation_h
                    in_w = ow_stride + min_w1[0] * dilation_w
                    if in_h < in_height and in_w < in_width:
                        T.atomic_add(
                            dweight_buffer[oc, min_ic[0], min_h1[0], min_w1[0]],
                            local_dout[0],
                        )
                    
                        T.atomic_add(
                            dimg_buffer[batch_idx, min_ic[0], in_h, in_w],
                            local_dout[0],
                        )
    ...
    def _kernel_multi(
        ...
        def compute_dinput_dweight_multi(
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P),
                ...
                final_neg_max = T.alloc_local((1,), dtype)
                min_ch_k = T.alloc_local((1,), int_dtype)
                ...
                if valid_b and valid_patch:
                    ...
                    for ic in T.serial(in_channels):
                        if neg_max_val[ic] > final_neg_max[0]:
                            final_neg_max[0] = neg_max_val[ic]
                            min_ch_k[0] = 1
                        elif neg_max_val[ic] == final_neg_max[0]:
                            min_ch_k[0] += 1
