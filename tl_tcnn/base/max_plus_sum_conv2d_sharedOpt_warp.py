"""Warp for MaxPlusSumConv2d.

Forward uses `max_plus_sum_conv2d_kernel` (NCHW input, OIHW weight, NCHW output).
...
from .max_plus_sum_conv2d_sharedOpt import (
    max_plus_sum_conv2d_kernel,
    max_plus_sum_conv2d_kernel_backward_dweight,
    max_plus_sum_conv2d_kernel_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return max_plus_sum_conv2d_kernel(*key)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return max_plus_sum_conv2d_kernel_backward_dweight(*key, maxmin_gradient=maxmin_gradient)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return max_plus_sum_conv2d_kernel_backward_dinput(*key, maxmin_gradient=maxmin_gradient)


def clear_max_plus_sum_conv2d_kernel_cache():
    """Clear Python-side compilation caches for MaxPlusSumConv2d kernels."""
...
class _MaxPlusSumConv2d(autograd.Function):
    """Autograd function for the MaxPlusSumConv2d operator."""
...
class MaxPlusSumConv2d(_Conv):
    ...
    def __init__(
        ...
        clear_max_plus_sum_conv2d_kernel_cache()
    ...
    def _conv_forward(
        ...
        output = _MaxPlusSumConv2d.apply(
            input,
            weight,
            self,
            input.shape[0],
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the MaxPlusSumConv2d layer."""
