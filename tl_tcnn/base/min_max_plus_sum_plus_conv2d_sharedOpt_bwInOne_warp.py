"""Warp for MinMaxPlusSumPlusConv2d.

Forward uses `min_max_plus_sum_plus_conv2d_kernel` (NCHW input, OIHW weight, NCHW output).
...
from .min_max_plus_sum_plus_conv2d_sharedOpt import (
    min_max_plus_sum_plus_conv2d_kernel,
    min_max_plus_sum_plus_conv2d_kernel_backward_dinput_dweight,
)
...
def _compile_forward_kernel_cached(*key):
    return min_max_plus_sum_plus_conv2d_kernel(*key)


@lru_cache(maxsize=1)
def _compile_backward_kernel_dinput_dweight_cached(*key, maxmin_gradient="multiple"):
    return min_max_plus_sum_plus_conv2d_kernel_backward_dinput_dweight(*key, maxmin_gradient=maxmin_gradient)

def clear_min_max_plus_sum_plus_conv2d_kernel_cache():
    """Clear Python-side compilation caches for MinMaxPlusSumPlusConv2d kernels."""
    _compile_forward_kernel_cached.cache_clear()
    _compile_backward_kernel_dinput_dweight_cached.cache_clear()


class _MinMaxPlusSumPlusConv2d(autograd.Function):
    """Autograd function for the MinMaxPlusSumPlusConv2d operator."""
...
class MinMaxPlusSumPlusConv2d(_Conv):
    ...
    def __init__(
        ...
        clear_min_max_plus_sum_plus_conv2d_kernel_cache()
    ...
    def _run_forward_kernel(self, input_nchw, weight_oihw, output, out_height, out_width):
        ...
        out_channels = weight_oihw.shape[0]
    ...
    def _run_backward_kernel(
        ...
        if key not in self.backward_kernels:
            with set_autotune_inputs(input_nchw, weight_oihw, grad_output_nchw, grad_input_nchw, grad_weight_oihw):
                self.backward_kernels[key] = _compile_backward_kernel_dinput_dweight_cached(
                    *key[:-1],
                    maxmin_gradient=maxmin_gradient,
                )
        self.backward_kernels[key](
            input_nchw,
            weight_oihw,
            grad_output_nchw,
            grad_input_nchw,
    ...
    def _conv_forward(
        ...
        if self.groups == 1:
            output = _MinMaxPlusSumPlusConv2d.apply(
                input,
                weight,
                self,
                input.shape[0],
            ...
            for g in range(self.groups):
                ...
                group_output = _MinMaxPlusSumPlusConv2d.apply(
                    group_input,
                    group_weight,
                    self,
                    input.shape[0],
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the MinMaxPlusSumPlusConv2d layer."""
