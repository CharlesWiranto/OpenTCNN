...
from .compound_min_max_plus_sum_conv2d2p import (
...
class CompoundMinMaxPlusSumConv2d2p_Pointwise(_Compound2p):
    ...
    def __init__(
        ...
        # Pointwise (k=1) variant uses per-OC scalar alpha/beta, not (OC, IC_per_group).
        self.alpha = Parameter(torch.empty((out_channels,), **factory_kwargs))
        self.beta = Parameter(torch.empty((out_channels,), **factory_kwargs))
