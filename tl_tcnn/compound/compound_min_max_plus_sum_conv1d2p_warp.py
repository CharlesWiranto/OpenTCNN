"""This is the warp file for kernels used for CompoundMinMaxPlusSumConv1d operation."""
...
from .compound_min_max_plus_sum_conv1d2p import (
    compound_min_max_plus_sum_conv1d2p_kernel,
    compound_min_max_plus_sum_conv1d2p_backward_dab,
    compound_min_max_plus_sum_conv1d2p_backward_dweight,
    compound_min_max_plus_sum_conv1d2p_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return compound_min_max_plus_sum_conv1d2p_kernel(
        *key,
    )
...
def _compile_backward_kernel_dab_cached(*key):
    return compound_min_max_plus_sum_conv1d2p_backward_dab(
        *key,
    )
...
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv1d2p_backward_dweight(
        *key,
...
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv1d2p_backward_dinput(
        *key,
        maxmin_gradient=maxmin_gradient,
    )


class _CompoundMinMaxPlusSumConv1d2p(autograd.Function):
    """Autograd function for the CompoundMinMaxPlusSumConv1d2p operator."""
    ...
    def forward(
        ...
        conv_module._run_forward_kernel(input_nchw, weight_oikk, alpha_oi, beta_oi, output, out_width)
    ...
    def backward(ctx, grad_output: Tensor):
        ...
        return (grad_input_nchw, grad_weight_oikk, grad_alpha_oi, grad_beta_oi) + (None,) * (1 + len(ctx.meta))

class CompoundMinMaxPlusSumConv1d2p(_Compound2p):
    """Compound Min-Max-Plus-Sum 1D Convolutional Layer with 2 Parameters."""
    ...
    def _conv_forward(
        ...
        if self.groups == 1:
            output = _CompoundMinMaxPlusSumConv1d2p.apply(
                input,
                weight,
                alpha,
                beta,
            ...
            for g in range(self.groups):
                ...
                group_output = _CompoundMinMaxPlusSumConv1d2p.apply(
                    group_input,
                    group_weight,
                    group_alpha,
                    group_beta,
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the CompoundMinMaxPlusSumConv1d2p layer."""
