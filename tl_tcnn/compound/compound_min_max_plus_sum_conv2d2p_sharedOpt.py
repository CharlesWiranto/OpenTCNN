...
def compound_min_max_plus_sum_conv2d2p_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
def compound_min_max_plus_sum_conv2d2p_backward_dab(
    ...
    """Shared-memory optimized gradient kernel for `dalpha` and `dbeta`."""
                ...
                for bhw_it in T.serial(T.ceildiv(BHW, reduce_threads)):
                    ...
                    if valid and bhw0 < BHW:
                        ...
                        if split_ties:
                            ...
                            if min_h[0] >= 0:
                                local_dw[min_h[0], min_w[0]] = local_dw[min_h[0], min_w[0]] + sh_beta[toc, tic] * sh_dout[tr, toc]
                    T.sync_threads()
...
# @tilelang.jit
def compound_min_max_plus_sum_conv2d2p_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def compound_min_max_plus_sum_conv2d2p_backward_dinput_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                                ...
                                if split_ties:
                                    ...
                                    if max_h[0] >= 0:
                                        ...
                                        if (ih1 >= 0) and (ih1 < in_height) and (iw1 >= 0) and (iw1 < in_width):
                                            ...
                                            T.atomic_add(dweight_buffer[oc, icg, max_h[0], max_w[0]], alpha_out[0])
                                    if min_h[0] >= 0:
                                        ih2 = oh * stride_h + min_h[0] * dilation_h
                                        iw2 = ow * stride_w + min_w[0] * dilation_w
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], beta_out[0])
                                            T.atomic_add(dweight_buffer[oc, icg, min_h[0], min_w[0]], beta_out[0])
...
# @tilelang.jit
def compound_min_max_plus_sum_conv2d2p_backward_dinput_dweight_dab(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                    ...
                    # for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                        ...
                        for ii in T.serial(TILE_IC):
                            ...
                            if icg < in_channels:
                                ...
                                if vmax[0] != -T.infinity(dtype):
                                    T.atomic_add(dalpha_buffer[oc, icg], vmax[0] * local_dout[0])
                                if vnegmax[0] != -T.infinity(dtype):
                                    T.atomic_add(dbeta_buffer[oc, icg], (-vnegmax[0]) * local_dout[0])
                                ...
                                if split_ties:
                                    ...
                                    for kh in T.serial(kernel_height):
                                        for kw in T.serial(kernel_width):
                                            if (vals[kh, kw] == vmax[0]) and (max_h[0] == -1):
                                                max_h[0] = kh
                                                max_w[0] = kw
                                            if (-vals[kh, kw] == vnegmax[0]) and (min_h[0] == -1):
                                                min_h[0] = kh
                                                min_w[0] = kw
                                    if max_h[0] >= 0:
                                        ih1 = oh * stride_h + max_h[0] * dilation_h
