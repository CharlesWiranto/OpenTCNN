...
# def compound_min_max_plus_sum_conv2d2p_kernel_tiled_shared(
#     batch_size,
#     in_channels,
#     out_channels,
#     in_height,
                    ...
                    for bhw_block in T.serial(T.ceildiv(BHW, BLOCK_BHW)):
                        ...
                        for bhw_offset in T.serial(TILE_BHW):
                            ...
                            if bhw < BHW:
                                ...
                                if max_argh[0] != -1:
                                    local_dweight_acc[max_argh[0], max_argw[0]] += (local_alpha[0] * local_dout[bhw_offset])
                                if min_argh[0] != -1:
                                    local_dweight_acc[min_argh[0], min_argw[0]] += (local_beta[0] * local_dout[bhw_offset])
                            ...
                            if bhw < BHW:
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        ...
                                        # Check bounds
                                        if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width):
                                            if (max_k[0] > 0) & (local_val[kh, kw] == max_val[0]):
...
def compound_min_max_plus_sum_conv2d2p_backward_dinput(
    ...
    the argmax/argmin locations of img + weight over (kh, kw), scaled by alpha and beta
    respectively.
    """    
    batches = batch_size
    patches = out_channels * out_height * out_width
                    ...
                    for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                        ...
                        for toh in T.serial(TILE_OH):
                            for tow in T.serial(TILE_OW):
                                ...
                                ow_out = base_ow + tow
                                if oh_out < out_height and ow_out < out_width:
                                    local_dout[0] = dout_buffer[batch_idx, oc, oh_out, ow_out]
                                    for ic in T.serial(TILE_IC):
                                        ic_global = bic * BLOCK_IC + tic * TILE_IC + ic
                                        if ic_global < in_channels:
                                            ...
                                            kh_sel2 = local_min_argh[0]
                                            kw_sel2 = local_min_argw[0]
                                            in_h1 = base_h + base_tih + kh_sel1 * dilation_h
                                            in_w1 = base_w + base_tiw + kw_sel1 * dilation_w
                                            in_h2 = base_h + base_tih + kh_sel2 * dilation_h
                                            in_w2 = base_w + base_tiw + kw_sel2 * dilation_w
                                            if (
                                                (in_h1 >= 0)
                                                and (in_h1 < in_height)
                                                and (in_w1 >= 0)
                                        ...
                                        if ic_global < in_channels:
                                            ...
                                            for kh in T.serial(kernel_height):
                                                ...
                                                for kw in T.serial(kernel_width):
                                                    ...
                                                    if in_h < in_height and in_w < in_width:
                                                        local_val[kh, kw] = (
                                                            local_img[ic, tih, tiw] + local_weight[ic, kh, kw]
                                                        )
                                                        if local_val[kh, kw] > local_max[0]:
                                                            local_max[0] = local_val[kh, kw]
                                                        if -local_val[kh, kw] > local_neg_max[0]:
                                            ...
                                            for kh in T.serial(kernel_height):
                                                for kw in T.serial(kernel_width):
                                                    in_h = base_h + base_tih + kh * dilation_h
                                                    in_w = base_w + base_tiw + kw * dilation_w
