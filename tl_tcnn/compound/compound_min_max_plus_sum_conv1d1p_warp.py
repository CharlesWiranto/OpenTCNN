...
from .compound_min_max_plus_sum_conv1d1p import (
    compound_min_max_plus_sum_conv1d1p_kernel,
    compound_min_max_plus_sum_conv1d1p_backward_da,
    compound_min_max_plus_sum_conv1d1p_backward_dweight,
    compound_min_max_plus_sum_conv1d1p_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return compound_min_max_plus_sum_conv1d1p_kernel(
        *key,
    )
...
def _compile_backward_kernel_da_cached(*key):
    return compound_min_max_plus_sum_conv1d1p_backward_da(
        *key,
    )
...
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv1d1p_backward_dweight(
        *key,
...
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv1d1p_backward_dinput(
        *key,
        maxmin_gradient=maxmin_gradient,
    )


class _CompoundMinMaxPlusSumConv1d1p(autograd.Function):
    """Autograd function for the CompoundMinMaxPlusSumConv1d1p operator."""
    ...
    def forward(
        ...
        conv_module._run_forward_kernel(input_nchw, weight_oikk, alpha_oi, output, out_width)
...
class CompoundMinMaxPlusSumConv1d1p(_Compound1p):
    ...
    def _conv_forward(
        ...
        if self.groups == 1:
            output = _CompoundMinMaxPlusSumConv1d1p.apply(
                input,
                weight,
                alpha,
                self,
            ...
            for g in range(self.groups):
                ...
                group_output = _CompoundMinMaxPlusSumConv1d1p.apply(
                    group_input,
                    group_weight,
                    group_alpha,
                    self,
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the CompoundMinMaxPlusSumConv1d1p layer."""
