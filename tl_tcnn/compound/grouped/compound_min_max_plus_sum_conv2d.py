                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    ...
                                    if (kmin[0] > 0) and (-vals[kh, kw] == vnegmax[0]):
                                        local_dw[kh, kw] = local_dw[kh, kw] + (
                                            sh_dout[tr, toc] / T.cast(kmin[0], dtype)
                                        )
                        else:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    ...
                                    if (vnegmax[0] != -T.infinity(dtype)) and (-vals[kh, kw] == vnegmax[0]) and (min_h[0] == -1):
                                        ...
                                        min_w[0] = kw
                            if max_h[0] >= 0:
                                local_dw[max_h[0], max_w[0]] = local_dw[max_h[0], max_w[0]] + sh_dout[tr, toc]
                            if min_h[0] >= 0:
                                local_dw[min_h[0], min_w[0]] = local_dw[min_h[0], min_w[0]] + sh_dout[tr, toc]
                    T.sync_threads()
