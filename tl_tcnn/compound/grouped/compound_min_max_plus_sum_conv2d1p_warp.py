...
def clear_compound2p_kernel_cache():
    ...
    _compile_backward_kernel_da_cached.cache_clear()
...
class _CompoundMinMaxPlusSumConv2d1p(autograd.Function):
    ...
    def backward(ctx, grad_output: Tensor):
        ...
        return (grad_input_nchw,
            grad_weight_oikk,
            grad_alpha_oi) + (None,) * (1 + len(ctx.meta))
            
class CompoundMinMaxPlusSumConv2d1p_Grouped(_Compound2p):
    ...
    def __getstate__(self):
        ...
        state.pop("backward_kernels_da", None)
