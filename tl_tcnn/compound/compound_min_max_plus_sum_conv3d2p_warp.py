...
from .compound_min_max_plus_sum_conv3d2p import (
    compound_min_max_plus_sum_conv3d2p_kernel,
    compound_min_max_plus_sum_conv3d2p_backward_dab,
    compound_min_max_plus_sum_conv3d2p_backward_dweight,
    compound_min_max_plus_sum_conv3d2p_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return compound_min_max_plus_sum_conv3d2p_kernel(
        *key,
    )
...
def _compile_backward_kernel_dab_cached(*key):
    return compound_min_max_plus_sum_conv3d2p_backward_dab(
        *key,
    )
...
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv3d2p_backward_dweight(
        *key,
...
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv3d2p_backward_dinput(
        *key,
        maxmin_gradient=maxmin_gradient,
    )


class _CompoundMinMaxPlusSumConv3d2p(autograd.Function):
    """Autograd function for the CompoundMinMaxPlusSumConv3d2p operator."""
    ...
    def forward(
        ...
        conv_module._run_forward_kernel(input_nchw, weight_oikk, alpha_oi, beta_oi, output, out_height, out_width, out_depth)
...
class CompoundMinMaxPlusSumConv3d2p(_Compound2p):
    ...
    def _conv_forward(
        ...
        if self.groups == 1:
            output = _CompoundMinMaxPlusSumConv3d2p.apply(
                input,
                weight,
                alpha,
                beta,
            ...
            for g in range(self.groups):
                ...
                group_output = _CompoundMinMaxPlusSumConv3d2p.apply(
                    group_input,
                    group_weight,
                    group_alpha,
                    group_beta,
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the CompoundMinMaxPlusSumConv3d2p layer."""
