...
from .compound_min_max_plus_sum_conv2d2p_sharedOpt import (
    ...
    compound_min_max_plus_sum_conv2d2p_backward_dinput_dweight,
)
...
def _compile_backward_kernel_dinput_dweight_cached(*key, maxmin_gradient="multiple"):
    return compound_min_max_plus_sum_conv2d2p_backward_dinput_dweight(
        *key,
...
class CompoundMinMaxPlusSumConv2d2p(_Compound2p):
    ...
    def _run_backward_kernel(
        ...
        if dinput_dweight_key not in self.backward_kernels_dinput_dweight:
            with set_autotune_inputs(
                input_nchw, weight_oikk, alpha_oi, beta_oi, 
                grad_output_nchw, grad_input_nchw, grad_weight_oikk
            ):
