                                ...
                                if oh_out < out_height and ow_out < out_width:
                                    for ic in T.serial(TILE_IC):
                                        ...
                                        if ic_global < in_channels:
                                            ...
                                            local_min[0] = -local_neg_max[0]
                                            local_accum[toh, tow] += local_alpha[ic] * (local_max[0] + local_min[0])
                    for toh in T.serial(TILE_OH):
                        for tow in T.serial(TILE_OW):
                    ...
                    for bhw_block in T.serial(T.ceildiv(BHW, BLOCK_BHW)):
                        ...
                        for bhw_offset in T.serial(TILE_BHW):
                            ...
                            if bhw < BHW:
                                ...
                                local_alpha_acc[0] = local_alpha_acc[0] + local_dout[bhw_offset] * (max_val[0] + min_val[0])
                    ...
                    for bhw_block in T.serial(T.ceildiv(BHW, BLOCK_BHW)):
                        ...
                        for bhw_offset in T.serial(TILE_BHW):
                            ...
                            if bhw < BHW:
                                ...
                                if min_argh[0] != -1:
                                    local_dweight2_acc[min_argh[0], min_argw[0]] += (local_alpha[0] * local_dout[bhw_offset])
...
def parallel_min_max_plus_sum_conv2d1p_backward_dinput(
    ...
    the argmax/argmin locations of img + weight1/2 over (kh, kw), scaled by alpha
    respectively.
    """    
    batches = batch_size
    patches = out_channels * out_height * out_width
