"""Shared-optimized implementation for ParallelMinMaxPlusSumConv2d2p kernels."""
...
def parallel_min_max_plus_sum_conv2d2p_kernel(
    ...
    def get_configs(*_kernel_params, **_kwargs):
        ...
        configs = []
        for block_oh, block_ow, r_threads in _configs:
    ...
    def _kernel(
        ...
        def main(
            ...
            with T.Kernel(bo_total, SPATIAL_TILES, threads=(BLOCK_P, reduce_threads)) as (bo, tile_id):
                ...
                sh_weight1 = T.alloc_shared((BLOCK_IC, kernel_height, kernel_width), dtype)
                sh_weight2 = T.alloc_shared((BLOCK_IC, kernel_height, kernel_width), dtype)
                sh_alpha = T.alloc_shared((BLOCK_IC,), dtype)
                sh_beta = T.alloc_shared((BLOCK_IC,), dtype)
                ...
                for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                    ...
                    for idx in T.serial(T.ceildiv(WEIGHT_ELEMS, total_threads)):
                        ...
                        if linear < WEIGHT_ELEMS:
                            ...
                            ic_global = bic * BLOCK_IC + ic_local
                            if valid_bo and ic_global < in_channels:
                                sh_weight1[ic_local, kh, kw] = weight1_buffer[oc, ic_global, kh, kw]
                                sh_weight2[ic_local, kh, kw] = weight2_buffer[oc, ic_global, kh, kw]
                            else:
                                sh_weight1[ic_local, kh, kw] = T.cast(0, dtype)
                                sh_weight2[ic_local, kh, kw] = T.cast(0, dtype)
                    ...
                    for idx in T.serial(T.ceildiv(BLOCK_IC, total_threads)):
                        ...
                        if ic_local < BLOCK_IC:
                            ...
                            if valid_bo and ic_global < in_channels:
                                ...
                                sh_beta[ic_local] = beta_buffer[oc, ic_global]
                            else:
                                sh_alpha[ic_local] = T.cast(0, dtype)
                                sh_beta[ic_local] = T.cast(0, dtype)
                    T.sync_threads()
                    if valid_out:
                        oh_off = toh * stride_h
                        ow_off = tow * stride_w
                        for ic in T.serial(TILE_IC):
                            ...
                            if ic_global < in_channels:
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        ...
                                        if in_tih < TILE_IH and in_tiw < TILE_IW:
                                            v1 = sh_img[ic_local, in_tih, in_tiw] + sh_weight1[ic_local, kh, kw]
                                            v2 = sh_img[ic_local, in_tih, in_tiw] + sh_weight2[ic_local, kh, kw]
                                            if v1 > local_max[0]:
                                                local_max[0] = v1
                                            if -v2 > local_neg_max[0]:
                                                local_neg_max[0] = -v2
                                ...
                                if local_min[0] != T.infinity(dtype):
                                    local_accum[0] += sh_beta[ic_local] * local_min[0]
                    T.sync_threads()
...
def parallel_min_max_plus_sum_conv2d2p_kernel_tiled(
    ...
    return parallel_min_max_plus_sum_conv2d2p_kernel(
        batch_size,
        in_channels,
        out_channels,
        in_height,
    ...
    def _dab_kernel(
        ...
        def compute_dalpha_dbeta(
            ...
            with T.Kernel(
                ...
                valid = (oc < out_channels) and (ic < in_channels)

                sh_weight1 = T.alloc_shared((BLOCK_OC, BLOCK_IC, kernel_height, kernel_width), dtype)
                sh_weight2 = T.alloc_shared((BLOCK_OC, BLOCK_IC, kernel_height, kernel_width), dtype)
                ...
                sh_dbeta = T.alloc_shared((BLOCK_OC, BLOCK_IC, reduce_threads), dtype)

                local_dalpha = T.alloc_local((1,), dtype)
                local_dbeta = T.alloc_local((1,), dtype)
                local_dalpha[0] = T.cast(0, dtype)
                local_dbeta[0] = T.cast(0, dtype)

                if valid:
                    for kh in T.serial(kernel_height):
                        for kw in T.serial(kernel_width):
                            sh_weight1[toc, tic, kh, kw] = weight1_buffer[oc, ic, kh, kw]
                            sh_weight2[toc, tic, kh, kw] = weight2_buffer[oc, ic, kh, kw]
                ...
                for bhw_it in T.serial(T.ceildiv(BHW, reduce_threads)):
                    ...
                    if valid and bhw0 < BHW:
                        ...
                        for kh in T.serial(kernel_height):
                            for kw in T.serial(kernel_width):
                                v1 = sh_img[tr, tic, kh, kw] + sh_weight1[toc, tic, kh, kw]
                                v2 = sh_img[tr, tic, kh, kw] + sh_weight2[toc, tic, kh, kw]
                                if v1 > vmax[0]:
                                    vmax[0] = v1
                                if -v2 > vnegmax[0]:
                                    vnegmax[0] = -v2
                        ...
                        if vmin[0] != T.infinity(dtype):
                            local_dbeta[0] = local_dbeta[0] + sh_dout[tr, toc] * vmin[0]
                    T.sync_threads()
                ...
                if valid:
                    ...
                    sh_dbeta[toc, tic, tr] = local_dbeta[0]

                T.sync_threads()

                if valid and tr == 0:
                    out_a = T.alloc_local((1,), dtype)
                    out_b = T.alloc_local((1,), dtype)
                    out_a[0] = T.cast(0, dtype)
                    out_b[0] = T.cast(0, dtype)
                    for r in T.serial(reduce_threads):
                        out_a[0] = out_a[0] + sh_dalpha[toc, tic, r]
                        out_b[0] = out_b[0] + sh_dbeta[toc, tic, r]
                    dalpha_buffer[oc, ic] = out_a[0]
                    dbeta_buffer[oc, ic] = out_b[0]
    ...
    def _kernel(
        ...
        def compute_dweight(
            ...
            with T.Kernel(
                ...
                sh_beta = T.alloc_shared((BLOCK_OC, BLOCK_IC), dtype)
                sh_img = T.alloc_shared((reduce_threads, BLOCK_IC, kernel_height, kernel_width), dtype)
                sh_dout = T.alloc_shared((reduce_threads, BLOCK_OC), dtype)
                sh_dw1 = T.alloc_shared((BLOCK_OC, BLOCK_IC, kernel_height, kernel_width, reduce_threads), dtype)
                sh_dw2 = T.alloc_shared((BLOCK_OC, BLOCK_IC, kernel_height, kernel_width, reduce_threads), dtype)

                local_dw1 = T.alloc_local((kernel_height, kernel_width), dtype)
                local_dw2 = T.alloc_local((kernel_height, kernel_width), dtype)
                T.clear(local_dw1)
                T.clear(local_dw2)
                ...
                if valid:
                    ...
                    sh_beta[toc, tic] = beta_buffer[oc, ic]
                ...
                for bhw_it in T.serial(T.ceildiv(BHW, reduce_threads)):
                    ...
                    if valid and bhw0 < BHW:
                        ...
                        vmax[0] = -T.infinity(dtype)
                        vnegmax[0] = -T.infinity(dtype)
                        vals1 = T.alloc_local((kernel_height, kernel_width), dtype)
                        vals2 = T.alloc_local((kernel_height, kernel_width), dtype)
                        for kh in T.serial(kernel_height):
                            for kw in T.serial(kernel_width):
                                vals1[kh, kw] = sh_img[tr, tic, kh, kw] + sh_weight1[toc, tic, kh, kw]
                                vals2[kh, kw] = sh_img[tr, tic, kh, kw] + sh_weight2[toc, tic, kh, kw]
                                if vals1[kh, kw] > vmax[0]:
                                    vmax[0] = vals1[kh, kw]
                                if -vals2[kh, kw] > vnegmax[0]:
                                    vnegmax[0] = -vals2[kh, kw]
                        ...
                        if split_ties:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (vmax[0] != -T.infinity(dtype)) and (vals1[kh, kw] == vmax[0]):
                                        kmax[0] += 1
                                    if (vnegmax[0] != -T.infinity(dtype)) and (-vals2[kh, kw] == vnegmax[0]):
                                        kmin[0] += 1
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (kmax[0] > 0) and (vals1[kh, kw] == vmax[0]):
                                        local_dw1[kh, kw] = local_dw1[kh, kw] + (
                                            sh_alpha[toc, tic] * sh_dout[tr, toc] / T.cast(kmax[0], dtype)
                                        )
                                    if (kmin[0] > 0) and (-vals2[kh, kw] == vnegmax[0]):
                                        local_dw2[kh, kw] = local_dw2[kh, kw] + (
                                            sh_beta[toc, tic] * sh_dout[tr, toc] / T.cast(kmin[0], dtype)
                                        )
                        else:
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    if (vmax[0] != -T.infinity(dtype)) and (vals1[kh, kw] == vmax[0]) and (max_h[0] == -1):
                                        max_h[0] = kh
                                        max_w[0] = kw
                                    if (vnegmax[0] != -T.infinity(dtype)) and (-vals2[kh, kw] == vnegmax[0]) and (min_h[0] == -1):
                                        min_h[0] = kh
                                        min_w[0] = kw
                            if max_h[0] >= 0:
                                local_dw1[max_h[0], max_w[0]] = local_dw1[max_h[0], max_w[0]] + sh_alpha[toc, tic] * sh_dout[tr, toc]
                            if min_h[0] >= 0:
                                local_dw2[min_h[0], min_w[0]] = local_dw2[min_h[0], min_w[0]] + sh_beta[toc, tic] * sh_dout[tr, toc]
                    T.sync_threads()
                ...
                if valid:
                    for kh in T.serial(kernel_height):
                        for kw in T.serial(kernel_width):
                            sh_dw1[toc, tic, kh, kw, tr] = local_dw1[kh, kw]
                            sh_dw2[toc, tic, kh, kw, tr] = local_dw2[kh, kw]
                ...
                if valid and tr == 0:
                    for kh in T.serial(kernel_height):
                        for kw in T.serial(kernel_width):
                            out1 = T.alloc_local((1,), dtype)
                            out2 = T.alloc_local((1,), dtype)
                            out1[0] = T.cast(0, dtype)
                            out2[0] = T.cast(0, dtype)
                            for r in T.serial(reduce_threads):
                                out1[0] = out1[0] + sh_dw1[toc, tic, kh, kw, r]
                                out2[0] = out2[0] + sh_dw2[toc, tic, kh, kw, r]
                            dweight1_buffer[oc, ic, kh, kw] = out1[0]
                            dweight2_buffer[oc, ic, kh, kw] = out2[0]
...
def parallel_min_max_plus_sum_conv2d2p_backward_dinput(
    ...
    def get_configs(*_kernel_params, **_kwargs):
        ...
        for boh, bow, rt in itertools.product(BLOCK_OH, BLOCK_OW, reduce_threads):
            ...
            block_ic = tile_ic * rt
            shared_elems = (
                block_ic * ((boh - 1) * stride_h + (kernel_height - 1) * dilation_h + 1)
                * ((bow - 1) * stride_w + (kernel_width - 1) * dilation_w + 1)
                + block_ic * kernel_height * kernel_width
                + block_ic * kernel_height * kernel_width
                + block_ic
                + block_ic
                ...
                for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                    ...
                    for idx in T.serial(T.ceildiv(WEIGHT_ELEMS, total_threads)):
                        ...
                        if linear < WEIGHT_ELEMS:
                            ...
                            kw = rem % kernel_width
                            icg = bic * BLOCK_IC + icl
                            if (bo < bo_total) and (icg < in_channels):
                                sh_weight1[icl, kh, kw] = weight1_buffer[oc, icg, kh, kw]
                                sh_weight2[icl, kh, kw] = weight2_buffer[oc, icg, kh, kw]
                            else:
                                sh_weight1[icl, kh, kw] = T.cast(0, dtype)
                                sh_weight2[icl, kh, kw] = T.cast(0, dtype)
                    ...
                    for idx in T.serial(T.ceildiv(BLOCK_IC, total_threads)):
                        ...
                        if icl < BLOCK_IC:
                            ...
                            if (bo < bo_total) and (icg < in_channels):
                                ...
                                sh_beta[icl] = beta_buffer[oc, icg]
                            else:
                                sh_alpha[icl] = T.cast(0, dtype)
                                sh_beta[icl] = T.cast(0, dtype)
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        ...
                                        if tih < TILE_IH and tiw < TILE_IW:
                                            vals1[kh, kw] = sh_img[icl, tih, tiw] + sh_weight1[icl, kh, kw]
                                            vals2[kh, kw] = sh_img[icl, tih, tiw] + sh_weight2[icl, kh, kw]
                                            if vals1[kh, kw] > vmax[0]:
                                                vmax[0] = vals1[kh, kw]
                                            if -vals2[kh, kw] > vnegmax[0]:
                                                vnegmax[0] = -vals2[kh, kw]
                                ...
                                if split_ties:
                                        ...
                                        for kw in T.serial(kernel_width):
                                            ...
                                            if (ih >= 0) and (ih < in_height) and (iw >= 0) and (iw < in_width):
                                                ...
                                                if (kmin[0] > 0) and (-vals2[kh, kw] == vnegmax[0]):
                                                    T.atomic_add(
                                                        dimg_buffer[b, icg, ih, iw],
                                                        sh_beta[icl] * local_dout[0] / T.cast(kmin[0], dtype),
                                                    )
                                else:
                                    max_h = T.alloc_local((1,), int_dtype)
                                    max_w = T.alloc_local((1,), int_dtype)
                                    ...
                                    if min_h[0] >= 0:
                                        ...
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], sh_beta[icl] * local_dout[0])
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d2p_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    return parallel_min_max_plus_sum_conv2d2p_backward_dinput(
        batch_size,
        in_channels,
        out_channels,
        in_height,
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d2p_backward_dinput_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                    ...
                    # for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                        ...
                        for ii in T.serial(TILE_IC):
                            ...
                            if icg < in_channels:
                                ...
                                vals1 = T.alloc_local((kernel_height, kernel_width), dtype)
                                vals2 = T.alloc_local((kernel_height, kernel_width), dtype)
                                alpha_out = T.alloc_local((1,), dtype)
                                alpha_out[0] = sh_alpha[icl] * local_dout[0]
                                beta_out = T.alloc_local((1,), dtype)
                                beta_out[0] = sh_beta[icl] * local_dout[0]
                                ...
                                if split_ties:
                                    ...
                                    for kh in T.serial(kernel_height):
                                        for kw in T.serial(kernel_width):
                                            ...
                                            if (vnegmax[0] != -T.infinity(dtype)) and (-vals2[kh, kw] == vnegmax[0]):
                                                kmin[0] += 1
                                    alpha_out_kmax = T.alloc_local((1,), dtype)
                                    beta_out_kmin = T.alloc_local((1,), dtype)
                                    if kmax[0] > 0:
                                        alpha_out_kmax[0] = alpha_out[0] / T.cast(kmax[0], dtype)
                                    else:
                                        alpha_out_kmax[0] = T.cast(0, dtype)
                                    if kmin[0] > 0:
                                        beta_out_kmin[0] = beta_out[0] / T.cast(kmin[0], dtype)
                                    else:
                                        beta_out_kmin[0] = T.cast(0, dtype)
                                    for kh in T.serial(kernel_height):
                                        for kw in T.serial(kernel_width):
                                            ih = oh * stride_h + kh * dilation_h
                                            iw = ow * stride_w + kw * dilation_w
                                    ...
                                    if max_h[0] >= 0:
                                        ...
                                        iw1 = ow * stride_w + max_w[0] * dilation_w
                                        if (ih1 >= 0) and (ih1 < in_height) and (iw1 >= 0) and (iw1 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih1, iw1], alpha_out[0])
                                            T.atomic_add(dweight1_buffer[oc, icg, max_h[0], max_w[0]], alpha_out[0])
                                    if min_h[0] >= 0:
                                        ih2 = oh * stride_h + min_h[0] * dilation_h
                                        iw2 = ow * stride_w + min_w[0] * dilation_w
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], beta_out[0])
                                            T.atomic_add(dweight2_buffer[oc, icg, min_h[0], min_w[0]], beta_out[0])

                    T.sync_threads()

        return compute_dinput_dweight
