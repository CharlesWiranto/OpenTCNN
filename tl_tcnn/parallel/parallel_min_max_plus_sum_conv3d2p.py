"""This is the file containing kernels used for ParallelMinMaxPlusSumConv3d operation."""
...
def parallel_min_max_plus_sum_conv3d2p_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    def _kernel(
        ...
        def main(
            ...
            weight1_buffer : T.Buffer((out_channels, in_channels, kernel_height, kernel_width, kernel_depth), dtype),
            weight2_buffer : T.Buffer((out_channels, in_channels, kernel_height, kernel_width, kernel_depth), dtype),
            ...
            with T.Kernel(T.ceildiv(batches, BLOCK_B), T.ceildiv(patches, BLOCK_P), 
                ...
                local_weight1 = T.alloc_local((TILE_IC, kernel_height, kernel_width, kernel_depth), dtype)
                local_weight2 = T.alloc_local((TILE_IC, kernel_height, kernel_width, kernel_depth), dtype)
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            local_weight1[ic, kh, kw, kd] = weight1_buffer[
                                                oc,
                                                ic_global,
                                                kh,
                                                kw,
                                            ...
                                            local_weight2[ic, kh, kw, kd] = weight2_buffer[
                                                oc,
                                                ic_global,
                                                kh,
                                                kw,
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            in_d = od_stride + kd * dilation_d
                                            if in_h < in_height and in_w < in_width and in_d < in_depth:
                                                val1 = (local_img[ic, kh, kw, kd] + local_weight1[ic, kh, kw, kd])
                                                val2 = (local_img[ic, kh, kw, kd] + local_weight2[ic, kh, kw, kd])
                                                if val1 > local_max[0]:
                                                    local_max[0] = val1
...
def parallel_min_max_plus_sum_conv3d2p_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                                    ...
                                    if oh_out < out_height and ow_out < out_width and od_out < out_depth:
                                        for ic in T.serial(TILE_IC):
                                            ...
                                            if ic_global < in_channels:
                                                ...
                                                for kh in T.serial(kernel_height):
                                                    ...
                                                    for kw in T.serial(kernel_width):
                                                        ...
                                                        for kd in T.serial(kernel_depth):
                                                            ...
                                                            val1 = (local_img[ic, tih, tiw, tid] + local_weight1[ic, kh, kw, kd])
                                                            val2 = (local_img[ic, tih, tiw, tid] + local_weight2[ic, kh, kw, kd])
                                                            if val1 > local_max[0]:
                                                                local_max[0] = val1
                                                            if -val2 > local_neg_max[0]:
                                                                local_neg_max[0] = -val2
                                                ...
                                                local_min[0] = -local_neg_max[0]
                                                local_accum[toh, tow, tod] += local_alpha[ic] * local_max[0] + local_beta[ic] * local_min[0]
                    for toh in T.serial(TILE_OH):
                        for tow in T.serial(TILE_OW):
                            for tod in T.serial(TILE_OD):
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv3d2p_backward_dab(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    dalpha[oc,ic] = Σ_{b,oh,ow,od} dout[b,oc,oh,ow,od] * max_val[oc,ic,b,oh,ow,od]
    ...
    def _dab_kernel(
        ...
        def compute_dalpha_dbeta(
            ...
            shared_dbeta = T.alloc_shared((BLOCK_OC, BLOCK_IC, BLOCK_BHWD), dtype)
            ...
            with T.Kernel(
                ...
                local_weight1 = T.alloc_local((kernel_height, kernel_width, kernel_depth), dtype)
                local_weight2 = T.alloc_local((kernel_height, kernel_width, kernel_depth), dtype)
                ...
                if valid_oc and valid_ic:
                    ...
                    for kh in T.serial(kernel_height):
                        for kw in T.serial(kernel_width):
                            for kd in T.serial(kernel_depth):
                                local_weight1[kh, kw, kd] = weight1_buffer[oc, ic, kh, kw, kd]
                                local_weight2[kh, kw, kd] = weight2_buffer[oc, ic, kh, kw, kd]
                    ...
                    for bhwd_block in T.serial(T.ceildiv(BHWD, BLOCK_BHWD)):
                        ...
                        for bhwd_offset in T.serial(TILE_BHWD):
                            ...
                            if bhwd < BHWD:
                                    ...
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width) & (in_d >= 0) & (in_d < in_depth):
                                                ...
                                                val1 = (local_img[bhwd_offset, kh, kw, kd] + local_weight1[kh, kw, kd])
                                                val2 = (local_img[bhwd_offset, kh, kw, kd] + local_weight2[kh, kw, kd])
                                ...
                                min_val[0] = -neg_max[0]
                                local_alpha_acc[0] = local_alpha_acc[0] + local_dout[bhwd_offset] * max_val[0]
                                local_beta_acc[0] = local_beta_acc[0] + local_dout[bhwd_offset] * min_val[0]
                ...
                if valid_oc and valid_ic:
                    ...
                    shared_dbeta[toc, tic, tbhwd] = local_beta_acc[0]
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv3d2p_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    dweight1[kh,kw,kd,ic,oc] = Σ_{b,oh,ow,od} (
        alpha[ic,oc] * dout[b,oh,ow,od,oc] * δ_max(ic,oc,b,oh,ow,od,kh,kw,kd) 
    )
    dweight2[kh,kw,kd,ic,oc] = Σ_{b,oh,ow,od} (
        beta[ic,oc] * dout[b,oh,ow,od,oc] * δ_min(ic,oc,b,oh,ow,od,kh,kw,kd)
    )
    ...
    def _kernel_dweight_single(
        ...
        def compute_dweight_single(
            ...
            beta_buffer: T.Buffer((out_channels, in_channels), dtype),
            dout_buffer: T.Buffer((batch_size, out_channels, out_height, out_width, out_depth), dtype),
            dweight1_buffer: T.Buffer((out_channels, in_channels, kernel_height, kernel_width, kernel_depth), dtype),
            dweight2_buffer: T.Buffer((out_channels, in_channels, kernel_height, kernel_width, kernel_depth), dtype),
        ):
            # Buffers
            ...
            with T.Kernel(
                ...
                shared_dweight1 = T.alloc_shared((BLOCK_OC, BLOCK_IC, kernel_height, kernel_width, kernel_depth, reduce_threads), dtype)
                shared_dweight2 = T.alloc_shared((BLOCK_OC, BLOCK_IC, kernel_height, kernel_width, kernel_depth, reduce_threads), dtype)
                
                # local memory
                local_dweight1_acc = T.alloc_local((kernel_height, kernel_width, kernel_depth), dtype)
                local_dweight2_acc = T.alloc_local((kernel_height, kernel_width, kernel_depth), dtype)
                    ...
                    for bhwd_block in T.serial(T.ceildiv(BHWD, BLOCK_BHWD)):
                        ...
                        for bhwd_offset in T.serial(TILE_BHWD):
                            ...
                            if bhwd < BHWD:
                                ...
                                if max_argh[0] != -1:
                                    local_dweight1_acc[max_argh[0], max_argw[0], max_argd[0]] += (local_alpha[0] * local_dout[bhwd_offset])
                                if min_argh[0] != -1:
                                    local_dweight2_acc[min_argh[0], min_argw[0], min_argd[0]] += (local_beta[0] * local_dout[bhwd_offset])

                    for kh in T.serial(kernel_height):
                        for kw in T.serial(kernel_width):
                            for kd in T.serial(kernel_depth):
                                shared_dweight1[toc, tic, kh, kw, kd, tbhwd] = local_dweight1_acc[kh, kw, kd]
                                shared_dweight2[toc, tic, kh, kw, kd, tbhwd] = local_dweight2_acc[kh, kw, kd]
                            # T.atomic_add(shared_dweight1[toc, tic, kh, kw, tbhwd], local_dweight1_acc[kh, kw])
                        ...
                        for kh in T.serial(kernel_height):
                            for kw in T.serial(kernel_width):
                                for kd in T.serial(kernel_depth):
                                    ...
                                    for r in T.serial(reduce_threads):
                                        total[0] = total[0] + shared_dweight1[toc, tic, kh, kw, kd, r]
                                        total[1] = total[1] + shared_dweight2[toc, tic, kh, kw, kd, r]
                                    dweight1_buffer[oc, ic, kh, kw, kd] = total[0] 
                                    dweight2_buffer[oc, ic, kh, kw, kd] = total[1]
    ...
    def _kernel_dweight_multi(
        ...
        def compute_dweight_multi(
            ...
            with T.Kernel(
                ...
                local_val = T.alloc_local((kernel_height, kernel_width, kernel_depth, 2), dtype)
                                    ...
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width) & (in_d >= 0) & (in_d < in_depth):
                                                # Use the locally cached per-position image value
                                                # to ensure identical behavior to buffer-based computation.
                                                local_val[kh, kw, kd, 0] = (local_img[bhwd_offset, kh, kw, kd] + local_weight1[kh, kw, kd])
                                                local_val[kh, kw, kd, 1] = (local_img[bhwd_offset, kh, kw, kd] + local_weight2[kh, kw, kd])
                                                
                                                if local_val[kh, kw, kd, 0] > max_val[0]:
                                                    max_val[0] = local_val[kh, kw, kd, 0]
                                                if -local_val[kh, kw, kd, 1] > neg_max[0]:
                                                    neg_max[0] = -local_val[kh, kw, kd, 1]
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            # Check bounds
                                            if (in_h >= 0) & (in_h < in_height) & (in_w >= 0) & (in_w < in_width) & (in_d >= 0) & (in_d < in_depth):
                                                if (max_k[0] > 0) & (local_val[kh, kw, kd, 0] == max_val[0]):
                                                    local_dweight1_acc[kh, kw, kd] += (
                                                        local_alpha[0]
                                                        * local_dout[bhwd_offset]
                                                        / T.cast(max_k[0], dtype)
                                                    )
                                                if (min_k[0] > 0) & (-local_val[kh, kw, kd, 1] == neg_max[0]):
                                                    local_dweight2_acc[kh, kw, kd] += (
                                                        local_beta[0]
                                                        * local_dout[bhwd_offset]
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv3d2p_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    Layouts:
    img     : (B, C, H, W, D) NCHWD
    weight1  : (OC, IC, KH, KW, KD) OIHKD
    weight2  : (OC, IC, KH, KW, KD) OIHKD
    alpha   : (OC, IC)
    beta    : (OC, IC)
    dout    : (B, OC, OH, OW, OD)
    dimg    : (B, C, H, W, D)
    Gradient: for each (b, oc, oh, ow, od) and input channel ic, contributions are routed to
    the argmax/argmin locations of img + weight1/2 over (kh, kw, kd), scaled by alpha and beta
    respectively.
    """    
    batches = batch_size
    patches = out_channels * out_height * out_width * out_depth
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            if in_h < in_height and in_w < in_width and in_d < in_depth:
                                                local_val[kh, kw, kd, 0] = (
                                                    local_img[ic, kh, kw, kd] + local_weight1[ic, kh, kw, kd]
                                                )
                                                local_val[kh, kw, kd, 1] = (
                                                    local_img[ic, kh, kw, kd] + local_weight2[ic, kh, kw, kd]
                                                )
                                                if local_val[kh, kw, kd, 0] > local_max[0]:
                                                    local_max[0] = local_val[kh, kw, kd, 0]
                                                if -local_val[kh, kw, kd, 1] > local_neg_max[0]:
                                                    local_neg_max[0] = -local_val[kh, kw, kd, 1]
                                ...
                                for kh in T.serial(kernel_height):
                                    for kw in T.serial(kernel_width):
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            if in_h < in_height and in_w < in_width and in_d < in_depth:
                                                if (max_k[0] > 0) and (local_val[kh, kw, kd, 0] == local_max[0]):
                                                    T.atomic_add(
                                                        dimg_buffer[batch_idx, ic_global, in_h, in_w, in_d],
                                                        local_alpha[ic] * local_dout[0] / T.cast(max_k[0], dtype),
                                                    )
                                                if (min_k[0] > 0) and (-local_val[kh, kw, kd, 1] == local_neg_max[0]):
                                                    T.atomic_add(
                                                        dimg_buffer[batch_idx, ic_global, in_h, in_w, in_d],
