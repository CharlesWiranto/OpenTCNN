"""Shared-optimized implementation for ParallelMinMaxPlusSumConv2d1p kernels."""
...
def parallel_min_max_plus_sum_conv2d1p_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
def parallel_min_max_plus_sum_conv2d1p_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d1p_backward_da(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d1p_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
            ...
            with T.Kernel(
                ...
                for bhw_it in T.serial(T.ceildiv(BHW, reduce_threads)):
                    ...
                    if valid and bhw0 < BHW:
                        ...
                        alpha_out[0] = sh_alpha[toc, tic] * sh_dout[tr, toc]
                            ...
                            for kh in T.serial(kernel_height):
                                for kw in T.serial(kernel_width):
                                    ...
                                    if (vnegmax[0] != -T.infinity(dtype)) and (-vals2[kh, kw] == vnegmax[0]) and (min_h[0] == -1):
                                        ...
                                        min_w[0] = kw
                            if max_h[0] >= 0:
                                local_dw1[max_h[0], max_w[0]] = local_dw1[max_h[0], max_w[0]] + alpha_out[0]
                            if min_h[0] >= 0:
                                local_dw2[min_h[0], min_w[0]] = local_dw2[min_h[0], min_w[0]] + alpha_out[0]
                    T.sync_threads()
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d1p_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                                ...
                                if split_ties:
                                    ...
                                    if min_h[0] >= 0:
                                        ...
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], alpha_out[0])
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d1p_backward_dinput_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv2d1p_backward_dinput_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                        ...
                        for ii in T.serial(TILE_IC):
                            ...
                            if icg < in_channels:
                                ...
                                if split_ties:
                                    ...
                                    alpha_out_kmin = T.alloc_local((1,), dtype)
                                    if kmax[0] > 0:
                                        alpha_out_kmax[0] = alpha_out[0] / T.cast(kmax[0], dtype)
                                    else:
                                        alpha_out_kmax[0] = T.cast(0, dtype)
                                    if kmin[0] > 0:
                                        alpha_out_kmin[0] = alpha_out[0] / T.cast(kmin[0], dtype)
                                    else:
                                        alpha_out_kmin[0] = T.cast(0, dtype)
                                    for kh in T.serial(kernel_height):
                                        for kw in T.serial(kernel_width):
                                            ih = oh * stride_h + kh * dilation_h
                                            iw = ow * stride_w + kw * dilation_w
                                    ...
                                    if min_h[0] >= 0:
                                        ...
                                        if (ih2 >= 0) and (ih2 < in_height) and (iw2 >= 0) and (iw2 < in_width):
                                            T.atomic_add(dimg_buffer[b, icg, ih2, iw2], alpha_out[0])
                                            T.atomic_add(dweight2_buffer[oc, icg, min_h[0], min_w[0]], alpha_out[0])
