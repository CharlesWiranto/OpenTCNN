...
from .parallel_min_max_plus_sum_conv1d1p import (
    parallel_min_max_plus_sum_conv1d1p_kernel,
    parallel_min_max_plus_sum_conv1d1p_backward_da,
    parallel_min_max_plus_sum_conv1d1p_backward_dweight,
    parallel_min_max_plus_sum_conv1d1p_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return parallel_min_max_plus_sum_conv1d1p_kernel(
        *key,
    )
...
def _compile_backward_kernel_da_cached(*key):
    return parallel_min_max_plus_sum_conv1d1p_backward_da(
        *key,
    )
...
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return parallel_min_max_plus_sum_conv1d1p_backward_dweight(
        *key,
...
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return parallel_min_max_plus_sum_conv1d1p_backward_dinput(
        *key,
        maxmin_gradient=maxmin_gradient,
    )


class _ParallelMinMaxPlusSumConv1d1p(autograd.Function):
    """Autograd function for the ParallelMinMaxPlusSumConv1d1p operator."""
    ...
    def forward(
        ...
        conv_module._run_forward_kernel(input_nchw, weight1_oikk, weight2_oikk, alpha_oi, output, out_width)
...
class ParallelMinMaxPlusSumConv1d1p(_Parallel1p):
    ...
    def _conv_forward(
        ...
        if self.groups == 1:
            output = _ParallelMinMaxPlusSumConv1d1p.apply(
                input,
                weight1,
                weight2,
                alpha,
            ...
            for g in range(self.groups):
                ...
                group_output = _ParallelMinMaxPlusSumConv1d1p.apply(
                    group_input,
                    group_weight1,
                    group_weight2,
                    group_alpha,
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the ParallelMinMaxPlusSumConv1d1p layer."""
