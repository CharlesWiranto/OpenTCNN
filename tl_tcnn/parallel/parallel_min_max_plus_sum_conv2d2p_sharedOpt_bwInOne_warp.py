"""SharedOpt warp file for ParallelMinMaxPlusSumConv2d2p kernels."""
...
from .parallel_min_max_plus_sum_conv2d2p_sharedOpt import (
    parallel_min_max_plus_sum_conv2d2p_kernel,
    parallel_min_max_plus_sum_conv2d2p_backward_dab,
    parallel_min_max_plus_sum_conv2d2p_backward_dinput_dweight,
)
...
def clear_parallel2p_kernel_cache():
    """Clear Python-side compilation caches for parallel2p sharedOpt kernels."""
    _compile_forward_kernel_cached.cache_clear()
    _compile_backward_kernel_dab_cached.cache_clear()
    _compile_backward_kernel_dinput_dweight_cached.cache_clear()


@lru_cache(maxsize=1)
def _compile_backward_kernel_dab_cached(*key):
    return parallel_min_max_plus_sum_conv2d2p_backward_dab(
        *key,
        # autotune=True,
    )
...
@lru_cache(maxsize=1)
def _compile_backward_kernel_dinput_dweight_cached(*key, maxmin_gradient="multiple"):
    return parallel_min_max_plus_sum_conv2d2p_backward_dinput_dweight(
        *key,
...
class _ParallelMinMaxPlusSumConv2d2p(autograd.Function):
    """Autograd function for the ParallelMinMaxPlusSumConv2d2p operator."""
    ...
    def forward(
        ...
        weight1_oikk = weight1.contiguous()
        weight2_oikk = weight2.contiguous()
...
class ParallelMinMaxPlusSumConv2d2p(_Parallel2p):
    """Parallel Min-Max-Plus-Sum 2D Convolutional Layer with 2 Parameters (SharedOpt)."""
    ...
    def __init__(
        ...
        self.backward_kernels_dinput_dweight = {}
    
    def __getstate__(self):
        state = self.__dict__.copy()
        state.pop("forward_kernels", None)
        state.pop("backward_kernels_dab", None)
        state.pop("backward_kernels_dinput_dweight", None)
        return state
    ...
    def _run_forward_kernel(
        ...
        out_channels = weight1_oikk.shape[0]
    ...
    def _run_backward_kernel(
        ...
        dinput_dweight_key = (*key, maxmin_gradient)
        if dinput_dweight_key not in self.backward_kernels_dinput_dweight:
            with set_autotune_inputs(
                input_nchw,
                weight1_oikk,
                weight2_oikk,
                ...
                self.backward_kernels_dinput_dweight[dinput_dweight_key] = _compile_backward_kernel_dinput_dweight_cached(
                    *key,
                    maxmin_gradient=maxmin_gradient,
                )
        self.backward_kernels_dinput_dweight[dinput_dweight_key](
            input_nchw,
            weight1_oikk,
            weight2_oikk,
            alpha_oi,
    ...
    def _conv_forward(
        ...
        padded_height, padded_width = input.shape[2], input.shape[3]
        out_height = (padded_height - self.dilation[0] * (self.kernel_size[0] - 1) - 1) // self.stride[0] + 1
        out_width = (padded_width - self.dilation[1] * (self.kernel_size[1] - 1) - 1) // self.stride[1] + 1

        if self.groups == 1:
            output = _ParallelMinMaxPlusSumConv2d2p.apply(
                input,
                weight1,
                weight2,
                alpha,
            ...
            for g in range(self.groups):
                ...
                group_output = _ParallelMinMaxPlusSumConv2d2p.apply(
                    group_input,
                    group_weight1,
                    group_weight2,
                    group_alpha,
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the ParallelMinMaxPlusSumConv2d2p layer."""
