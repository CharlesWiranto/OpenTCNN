"""This is the warp file for kernels used for ParallelMinMaxPlusSumConv3d operation."""
...
from .parallel_min_max_plus_sum_conv3d2p import (
    parallel_min_max_plus_sum_conv3d2p_kernel,
    parallel_min_max_plus_sum_conv3d2p_backward_dab,
    parallel_min_max_plus_sum_conv3d2p_backward_dweight,
    parallel_min_max_plus_sum_conv3d2p_backward_dinput,
)
...
def _compile_forward_kernel_cached(*key):
    return parallel_min_max_plus_sum_conv3d2p_kernel(
        *key,
    )
...
def _compile_backward_kernel_dab_cached(*key):
    return parallel_min_max_plus_sum_conv3d2p_backward_dab(
        *key,
    )
...
def _compile_backward_kernel_dweight_cached(*key, maxmin_gradient="multiple"):
    return parallel_min_max_plus_sum_conv3d2p_backward_dweight(
        *key,
...
def _compile_backward_kernel_dinput_cached(*key, maxmin_gradient="multiple"):
    return parallel_min_max_plus_sum_conv3d2p_backward_dinput(
        *key,
        maxmin_gradient=maxmin_gradient,
    )


class _ParallelMinMaxPlusSumConv3d2p(autograd.Function):
    """Autograd function for the ParallelMinMaxPlusSumConv3d2p operator."""
    ...
    def forward(
        ...
        conv_module._run_forward_kernel(input_nchw, weight1_oikk, weight2_oikk, alpha_oi, beta_oi, output, out_height, out_width, out_depth)
...
class ParallelMinMaxPlusSumConv3d2p(_Parallel2p):
    """Parallel Min-Max-Plus-Sum 3D Convolutional Layer with 2 Parameters."""
    ...
    def _conv_forward(
        ...
        if self.groups == 1:
            output = _ParallelMinMaxPlusSumConv3d2p.apply(
                input,
                weight1,
                weight2,
                alpha,
            ...
            for g in range(self.groups):
                ...
                group_output = _ParallelMinMaxPlusSumConv3d2p.apply(
                    group_input,
                    group_weight1,
                    group_weight2,
                    group_alpha,
    ...
    def forward(self, input: Tensor) -> Tensor:
        """Forward function for the ParallelMinMaxPlusSumConv3d2p layer."""
