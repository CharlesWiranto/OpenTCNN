"""This is the file containing kernels used for ParallelMinMaxPlusSumConv1d1p operation."""
...
def parallel_min_max_plus_sum_conv1d1p_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_width,
...
def parallel_min_max_plus_sum_conv1d1p_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_width,
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv1d1p_backward_da(
    batch_size,
    in_channels,
    out_channels,
    in_width,
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv1d1p_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_width,
    ...
    dweight2[kh,kw,ic,oc] = Σ_{b,oh,ow} (
        alpha[ic,oc] * dout[b,oh,ow,oc] * δ_min(ic,oc,b,oh,ow,kh,kw)
    )
                    ...
                    for bw_block in T.serial(T.ceildiv(BW, BLOCK_BW)):
                        ...
                        for bw_offset in T.serial(TILE_BW):
                            ...
                            if bw < BW:
                                ...
                                if min_argw[0] != -1:
                                    local_dweight2_acc[min_argw[0]] += (local_alpha[0] * local_dout[bw_offset])
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv1d1p_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_width,
    ...
    the argmax/argmin locations of img + weight1/2 over (kw), scaled by alpha
    respectively.
    """    
    batches = batch_size
    patches = out_channels * out_width
