...
def parallel_min_max_plus_sum_conv3d1p_kernel(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                    ...
                    for bic in T.serial(T.ceildiv(in_channels, BLOCK_IC)):
                        ...
                        for ic in T.serial(TILE_IC):
                            ...
                            if ic_global < in_channels:
                                ...
                                local_accum[0] += local_alpha[ic] * (local_max[0] + local_min[0])
...
def parallel_min_max_plus_sum_conv3d1p_kernel_tiled(
    batch_size,
    in_channels,
    out_channels,
    in_height,
                                    ...
                                    if oh_out < out_height and ow_out < out_width and od_out < out_depth:
                                        for ic in T.serial(TILE_IC):
                                            ...
                                            if ic_global < in_channels:
                                                ...
                                                local_min[0] = -local_neg_max[0]
                                                local_accum[toh, tow, tod] += local_alpha[ic] * (local_max[0] + local_min[0])
                    for toh in T.serial(TILE_OH):
                        for tow in T.serial(TILE_OW):
                            for tod in T.serial(TILE_OD):
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv3d1p_backward_da(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    dalpha[oc,ic] = Σ_{b,oh,ow,od} dout[b,oc,oh,ow,od] * (max_val[oc,ic,b,oh,ow,od] + min_val[oc,ic,b,oh,ow])
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv3d1p_backward_dweight(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    dweight2[kh,kw,kd,ic,oc] = Σ_{b,oh,ow,od} (
        alpha[ic,oc] * dout[b,oh,ow,od,oc] * δ_min(ic,oc,b,oh,ow,od,kh,kw,kd)
    )
                    ...
                    for bhwd_block in T.serial(T.ceildiv(BHWD, BLOCK_BHWD)):
                        ...
                        for bhwd_offset in T.serial(TILE_BHWD):
                            ...
                            if bhwd < BHWD:
                                ...
                                if min_argh[0] != -1:
                                    local_dweight2_acc[min_argh[0], min_argw[0], min_argd[0]] += (local_alpha[0] * local_dout[bhwd_offset])
...
# @tilelang.jit
def parallel_min_max_plus_sum_conv3d1p_backward_dinput(
    batch_size,
    in_channels,
    out_channels,
    in_height,
    ...
    the argmax/argmin locations of img + weight1/2 over (kh, kw, kd), scaled by alpha
    respectively.
    """    
    batches = batch_size
    patches = out_channels * out_height * out_width * out_depth
                                        ...
                                        for kd in T.serial(kernel_depth):
                                            ...
                                            if in_h < in_height and in_w < in_width and in_d < in_depth:
                                                ...
                                                if (min_k[0] > 0) and (-local_val[kh, kw, kd, 1] == local_neg_max[0]):
                                                    T.atomic_add(
                                                        dimg_buffer[batch_idx, ic_global, in_h, in_w, in_d],
                                                        local_alpha[ic] * local_dout[0] / T.cast(min_k[0], dtype),
                                                    )
