...
def count_operations_in_model(model, inputs):
    ...
    def count_operations(module, input, output):
        ...
        if isinstance(module, nn.Linear):
            ...
            module,
            (
                tlayers.MaxPlusMaxConv1d,
                tlayers.MaxPlusMaxConv2d,
                tlayers.MaxPlusMaxConv3d,
                tlayers.MaxPlusMinConv1d,
                tlayers.MaxPlusMinConv2d,
                tlayers.MaxPlusMinConv3d,
                tlayers.MinPlusMaxConv1d,
                tlayers.MinPlusMaxConv2d,
                tlayers.MinPlusMaxConv3d,
                tlayers.MinPlusMinConv1d,
                tlayers.MinPlusMinConv2d,
                tlayers.MinPlusMinConv3d,
            ),
        ):
            # Max-Plus卷积层中的最大值和加法
            input_channels = module.in_channels
            ...
            module,
            (
                tlayers.MaxPlusSumConv1d,
                tlayers.MaxPlusSumConv2d,
                tlayers.MaxPlusSumConv3d,
                tlayers.MinPlusSumConv1d,
                tlayers.MinPlusSumConv2d,
                tlayers.MinPlusSumConv3d,
            ),
        ):
            # Max-Plus卷积层中的最大值和加法
            input_channels = module.in_channels
            ...
            output_elements = output.numel()
            add_count += (
                (input_channels - 1 + num_elements_per_kernel * input_channels)
                * output_channels
                * output_elements
            )
            compare_count += (
            ...
            module,
            (
                tlayers.CompoundMinMaxPlusSumConv1d1p,
                tlayers.CompoundMinMaxPlusSumConv1d2p,
                tlayers.CompoundMinMaxPlusSumConv2d1p,
                tlayers.CompoundMinMaxPlusSumConv2d2p,
                tlayers.CompoundMinMaxPlusSumConv3d1p,
                tlayers.CompoundMinMaxPlusSumConv3d2p,
            ),
        ):
            # 复合卷积层中的最大值、最小值和加法
            input_channels = module.in_channels
            ...
            add_count += (
                (num_elements_per_kernel * input_channels + 2 * input_channels - 1)
                * output_channels
                * output_elements
            )
            compare_count += (
            ...
            module,
            (
                tlayers.ParallelMinMaxPlusSumConv1d1p,
                tlayers.ParallelMinMaxPlusSumConv1d2p,
                tlayers.ParallelMinMaxPlusSumConv2d1p,
                tlayers.ParallelMinMaxPlusSumConv2d2p,
                tlayers.ParallelMinMaxPlusSumConv3d1p,
                tlayers.ParallelMinMaxPlusSumConv3d2p,
            ),
        ):
            # 并行卷积层中的最大值、最小值和加法
            input_channels = module.in_channels
            ...
            mul_count += 2 * input_channels * output_channels * output_channels
            add_count += (
                (2 * num_elements_per_kernel * input_channels + 2 * input_channels - 1)
                * output_channels
                * output_elements
            )
            compare_count += (
    ...
    return (
        ...
        add_count,
        compare_count,
        linear_layers_mul_count,
        linear_layers_add_count,
    )
...
def count_residual_additions(model, input_tensor):
    ...
    def count_residual_addition(module, input, output, name):
        ...
        if hasattr(module, "downsample") or isinstance(
            module,
            (
                models.resnet.BasicBlock,
                models.resnet.Bottleneck,
                t_models.Compound2pBasicBlock1,
                t_models.Compound2pBasicBlock2,
                t_models.Parallel2pBasicBlock1,
                t_models.Parallel2pBasicBlock2,
            ),
        ):
        ...
        if isinstance(
            ...
            hooks.append(
                layer.register_forward_hook(
                    lambda module, input, output, name=name: count_residual_addition(
